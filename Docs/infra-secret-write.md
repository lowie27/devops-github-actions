...
change its definition. Point `write-environment` at an environment with a protected-branch policy so
the definition being run is one that went through review.
...
Only `scaleway` works, and the other two cannot be added by writing another branch of a `case`.
...
- **Azure Key Vault** holds one value per secret. The read-merge-write design, the concurrency lock
  and the do-not-drop-the-other-keys guarantee all exist because Scaleway models this as a single
  `key_value` blob. Supporting Key Vault means a different algorithm, not a different backend.
