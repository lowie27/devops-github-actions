...
Wrong region is the usual cause: the filter returns an empty list rather than an error." ;;
	...
	# scheduled for deletion, so revision=latest would 404 on a count that looks non-zero. Filtered
	# here rather than with status.0=enabled, which the CLI accepts and ignores.
	local versions
...
every other key in this secret stops being synced"
	...
	json=$(scw secret version access "$SECRET_ID" revision=latest region="$REGION" -o json 2>&1) \
...
assert_flat_string_object() {
	...
	# Keys become env var names in the pod through envFrom. A key holding a newline also desyncs the
	# four-line summary read in cmd_set, which would then print a confident, wrong summary.
	bad=$(jq -r '[keys[] | select(test("^[A-Za-z_][A-Za-z0-9_]*$") | not) | @json] | join(", ")' "$file")
	[[ -z "$bad" ]] || die "$what has key(s) that are not valid env var names: $bad"
}
...
cmd_set() {
	...
	fetch_latest "$old"
	assert_flat_string_object "$old" "current version of $name"
